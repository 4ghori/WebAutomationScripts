...
public class MSGOnlineDigitalSauceJobAccessController implements Serializable {
	/**
	 * Logger instance.
	 */
	private static final Logger logger = Logger.getLogger(
			MSGOnlineDigitalSauceJobAccessController.class.getName());
	/**
	 * 10 seconds in milliseconds.
	 */
	private static final long HTTP_READ_TIMEOUT_SECONDS = TimeUnit.SECONDS
			.toMillis(10);
	/**
	 * 10 seconds in milliseconds.
	 */
	private static final long HTTP_CONNECT_TIMEOUT_SECONDS = TimeUnit.SECONDS
			.toMillis(10);
	/**
	...
	static {
		if (System.getenv("SAUCE_REST_ENDPOINT") != null) {
			...
			BASE_URL = System.getProperty("saucerest-java.base_url",
					"https://saucelabs.com/");
		}
	}
	...
	/**
	 * The access key to use when performing HTTP requests to the Sauce REST
	 * API.
	 */
	 ...
	 *
	 * @param username
	 *            The username to use when performing HTTP requests to the Sauce
	 *            REST API
	 * @param accessKey
	 *            The access key to use when performing HTTP requests to the
	 *            Sauce REST API
	 */
	public MSGOnlineDigitalSauceJobAccessController(String username,
			String accessKey) {
	...
	public static void setExtraUserAgent(String extraUserAgent) {
		MSGOnlineDigitalSauceJobAccessController.extraUserAgent = extraUserAgent;
	}
	 ...
	 *
	 * @param endpoint
	 *            Endpoint url, example "info/platforms/appium"
	 *
	...
	public String doJSONPOST(URL url, JSONObject body)
			throws MSGOnlineDigitalSauceException {
		...
		try {
			...
			if (postBack instanceof HttpsURLConnection) {
				MSGOnlineDigitalSauceSSLSocketFactory factory = new MSGOnlineDigitalSauceSSLSocketFactory();
			...
			reader = new BufferedReader(
					new InputStreamReader(postBack.getInputStream()));
			...
			try {
				if (postBack.getResponseCode() == 401) {
					throw new MSGOnlineDigitalSauceException.NotAuthorized();
				}
			} catch (IOException e1) {
				logger.log(Level.SEVERE, "Error POSTing to " + url.toString()
						+ " and getting status code: ", e);
			}
			...
			try {
				...
				logger.log(Level.WARNING, "Error closing Sauce input stream",
						e);
			}
		}
	 ...
	 *            sessionId
	 * @param location
	 *            represents the base directory where the video should be
	 *            downloaded to
	 */
	public void downloadVideo(String jobId, String location) {
		URL restEndpoint = this.buildURL(
				"v1/" + username + "/jobs/" + jobId + "/assets/video.flv");
	...
	/**
	 * Returns the HTTP response for invoking
	 * https://saucelabs.com/rest/v1/path.
	 *
	 * @param path
	...
	/**
	 * Returns a String (in JSON format) representing the details for a Sauce
	 * job.
	 *
	 * @param jobId
	 *            the Sauce Job id to retrieve
	 *
	...
	public String getFullJobs(int limit) {
		URL restEndpoint = this
				.buildURL("v1/" + username + "/jobs?full=true&limit=" + limit);
		return retrieveResults(restEndpoint);
	}
	 ...
	 *
	 * @param limit
	 *            Number of jobs to return(max of 500)
	 *
	 * @return String (in JSON format) representing the jobID for a sauce Job
	 */
	public String getJobs(int limit) {
		URL restEndpoint = this
				.buildURL("v1/" + username + "/jobs?limit=" + limit);
		return retrieveResults(restEndpoint);
	}
	 ...
	 *            Number of jobs to return(max of 500)
	 * @param to
	 *            value in Epoch time format denoting the time to end the job
	 *            list searh
	 * @param from
	 *            value in Epoch time format denoting the time to start the
	 *            search
	 *
	 * @return String (in JSON format) representing the jobID for a sauce Job
	 */
	public String getJobs(int limit, long to, int from) {
		URL restEndpoint = this.buildURL("v1/" + username + "/jobs?limit="
				+ limit + "&from=" + to + "&to=" + from);
		return retrieveResults(restEndpoint);
	}

	/**
	 * @param restEndpoint
	 *            the URL to perform a HTTP GET
	 *
	 * @return Returns the response from invoking a HTTP GET for the
	 *         restEndpoint
	 */
	public String retrieveResults(URL restEndpoint) {
		...
		try {
			...
			logger.log(Level.SEVERE,
					"Received a SocketTimeoutException when invoking Sauce REST API, check status.saucelabs.com for network outages",
					e);
		} catch (IOException e) {
	...
	/**
	 * Stores the result of a HTTP GET to the value of the
	 * <code>restEndpoint</code> parameter, saving the resulting file to the
	 * directory defined by the <code>location</code> parameter.
	 *
	 * @param jobId
	 *            the Sauce Job id
	 * @param location
	 *            represents the location that the result file should be stored
	 *            in
	 * @param restEndpoint
	 *            the URL to perform a HTTP GET
	 */
	 ...
	 *
	 * @param connection
	 *            HttpURLConnection instance which represents the current HTTP
	 *            request
	 */
	...
	/**
	 * Invokes the Sauce REST API to update the details of a Sauce job, using
	 * the details included in the <code>updates</code> parameter.
	 *
	 * @param jobId
	 *            the Sauce job id to update
	 * @param updates
	...
	public void updateJobInfo(String jobId, Map<String, Object> updates)
		...
		try {
			URL restEndpoint = this
					.buildURL("v1/" + username + "/jobs/" + jobId);
	...
	public HashMap<String, String> getUserActivity(Map<String, Object> updates)
			throws IOException {
		HashMap response = new HashMap<Object, Object>();
		HttpURLConnection getUA = null;
		try {
			URL restEndpoint = this
					.buildURL("v1/" + username + "/jobs?full=true");
			getUA = openConnection(restEndpoint);
			getUA.setRequestProperty("User-Agent", this.getUserAgent());
			getUA.setDoOutput(true);
			getUA.setRequestMethod("GET");
			addAuthenticationProperty(getUA);
			String jsonText = JSONValue.toJSONString(updates);
			response.put("userActivity", "");
		} catch (IOException e) {
			logger.log(Level.WARNING, "Error getting Sauce User Activity", e);
		}
		closeInputStream(getUA);
		return response;
	}
	...
	public void stopJob(String jobId) {
		...
		try {
			URL restEndpoint = this
					.buildURL("v1/" + username + "/jobs/" + jobId + "/stop");
	 ...
	 *
	 * @param file
	 *            the file to upload -param fileName uses file.getName() to
	 *            store in sauce -param overwrite set to true
	 *
	 * @return the md5 hash returned by sauce of the file
	 *
	 * @throws IOException
	 *             can be thrown when server returns an error (tcp or http
	 *             status not in the 200 range)
	 */
	 ...
	 *            the file to upload
	 * @param fileName
	 *            name of the file in sauce storage -param overwrite set to true
	 *
	 * @return the md5 hash returned by sauce of the file
	 *
	 * @throws IOException
	 ...
	 *            the file to upload
	 * @param fileName
	 *            name of the file in sauce storage
	 * @param overwrite
	 *            boolean flag to overwrite file in sauce storage if it exists
	 *
	 * @return the md5 hash returned by sauce of the file
	 *
	 * @throws IOException
	 ...
	 *             status not in the 200 range)
	 */
	public String uploadFile(File file, String fileName, Boolean overwrite)
			throws IOException {
	 ...
	 *
	 * @param is
	 *            Input stream of the file to be uploaded
	 * @param fileName
	 *            name of the file in sauce storage
	 * @param overwrite
	 *            boolean flag to overwrite file in sauce storage if it exists
	 ...
	 *             status not in the 200 range)
	 */
	public String uploadFile(InputStream is, String fileName, Boolean overwrite)
			throws IOException {
		try {
			URL restEndpoint = this.buildURL("v1/storage/" + username + "/"
					+ fileName + "?overwrite=" + overwrite.toString());
			...
			if (sauceUploadResponse.has("error")) {
				throw new UnexpectedException(
						"Failed to upload to sauce-storage: "
	 ...
	 * the key &lt;username&gt;:&lt;api key&gt; (see <a href=
	 * "http://saucelabs.com/docs/integration#public-job-links">http://saucelabs.com/docs/integration#public-job-links</a>).
	 *
	 * @param jobId
	 *            the Sauce Job Id, typically equal to the Selenium/WebDriver
	 *            sessionId
	...
	public String getPublicJobLink(String jobId) {
		try {
			...
			String auth_token = MSGOnlineDigitalSecurityUtils
					.hmacEncode("HmacMD5", jobId, key);
			return "https://saucelabs.com/jobs/" + jobId + "?auth="
					+ auth_token;
		} catch (IllegalArgumentException ex) {
			// someone messed up on the algorithm to hmacEncode
			// For available algorithms see {@link
			// http://docs.oracle.com/javase/7/docs/api/javax/crypto/Mac.html}
			// we only want to use 'HmacMD5'
			logger.log(Level.WARNING,
					"Unable to create an authenticated public link to job:",
					ex);
			return "";
		}
	}
	 ...
	 *
	 * @param tunnelId
	 *            Identifier of the tunnel to delete
	 */
	public void deleteTunnel(String tunnelId) {
		...
		try {
			URL restEndpoint = this
					.buildURL("v1/" + username + "/tunnels/" + tunnelId);
	...
	/**
	 * Invokes the Sauce REST API to retrieve the details of the tunnels
	...
	/**
	 * Invokes the Sauce REST API to retrieve the concurrency details of the
	 * user.
	 *
	...
	public String getConcurrency() {
		URL restEndpoint = this
				.buildURL("v1/users/" + username + "/concurrency");
		return retrieveResults(restEndpoint);
	}
	...
	/**
	 * Returns a String (in JSON format) representing the basic account
	 * information
	 *
	 * @return String (in JSON format) representing the basic account
	 *         information
	 */
	...
	/**
	 * Returns a String (in JSON format) representing the list of objects
	 * describing all the OS and browser platforms currently supported on Sauce
	 * Labs. (see <a href=
	 * "https://docs.saucelabs.com/reference/rest-api/#get-supported-platforms">https://docs.saucelabs.com/reference/rest-api/#get-supported-platforms</a>).
	 *
	 * @param automationApi
	 *            the automation API name
	 *
	 * @return String (in JSON format) representing the supported platforms
	 *         information
	 */
	 ...
	 *
	 * @param platform
	 *            Platform string. Such as "jenkins", "bamboo", "teamcity"
	 * @param platformVersion
	 *            Version string. Such as "1.1.1"
	 *
	...
	public boolean recordCI(String platform, String platformVersion) {
		...
		} catch (MSGOnlineDigitalSauceException e) {
			return false;
		}
		return true;
	}
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof MSGOnlineDigitalSauceJobAccessController)) {
			return super.equals(obj);
		}
		MSGOnlineDigitalSauceJobAccessController sauceobj = (MSGOnlineDigitalSauceJobAccessController) obj;
