    ...
    public String generateJWT(String requestMethod, URI uri, final int jwtExpiryWindowSeconds) {
     ...
     //   final int jwtExpiryWindowSeconds = 60 * 3;
        final JwtAuthorizationGenerator jwtAuthorisationGenerator = new JwtAuthorizationGenerator(jwtWriterFactory, jwtExpiryWindowSeconds) {
            ...
            private String issueJwt(String jsonPayload, ZephyrConfig config) throws JwtSigningException, JwtIssuerLacksSharedSecretException, JwtUnknownIssuerException {
                return getJwtWriter(config).jsonToJwt(jsonPayload);
            }

            private JwtWriter getJwtWriter(ZephyrConfig config) throws JwtUnknownIssuerException, JwtIssuerLacksSharedSecretException {
