...
public class Mailinator {
    ...
    public static Email getEmail(String apikey, String emailId, boolean privateDomain) throws IOException {
        ...
        //System.out.println(obj.get("body").toString()); 
    ...
    public static String getEmailBody(String apikey, String emailId, boolean privateDomain) throws  IOException
    {
    	String emailBodytext="";
    	
    	//===>> Format The API END pint URL as per MAILINATOR  Requirement Hit the End-Point and retrieve the data as JSONObject
        	String emailUrl = String.format(MAILINATOR_EMAIL_TEMPLATE_URL, apikey, emailId, privateDomain);
        	Reader reader = getResponse(emailUrl);
        	JSONObject jsonEmail = (JSONObject) JSONValue.parse(reader);
    	
        //==>> Parse the Data using a custom Email Class created in Email.java file of MAILINATOR Project
        	//Email emailMsg = new Email();
        	//emailMsg.setApiEmailFetchesLeft(Integer.valueOf(jsonEmail.get("apiEmailFetchesLeft").toString()));  // Dont know what is this
        	
        	JSONObject jsonDataSection = (JSONObject) jsonEmail.get("data");  // The Response comes as DATA so reading the entire Json here 
	        ...
	        for (Object jsonPart1 : jsonParts) 
	        {
	            ...
	            emailBodytext=jsonPart.get("body").toString();
    ...
    private static Email createEmailFrom(JSONObject jsonEmail) {
        ...
        for (Object jsonPart1 : jsonParts) {
            ...
            emailPart.setBody(jsonPart.get("body").toString());
            System.out.println(emailPart.getBody());
